...
export default function PDF() {
    ...
    const { CartItem, totalQuantity, totalAmount, cgst, sgst } = useSelector(state => state.cart);
    ...
    return (
        <div>
            <div ref={targetRef} className='pdf_section'>
                ...
                <h5 className=' text-center'>GST:- <span>24AASFJ0418NI21</span></h5>
                <p className=' text-center'>DT:- <span>{CurrentDate.toLocaleDateString()}</span> TM:- <span>{CurrentTime}</span></p>
                ...
                <table className=' table '>
                    ...
                    <tbody>
                        {
                            CartItem && CartItem.map(item => <Tr key={item.id} item={item}></Tr>)
                        }
                    </tbody>
                </table>

                <div className='NItem_NQty d-flex justify-content-around'>
                    <p>NItem:- <span>{itemLenght}</span></p>
                    <p>NQty:- <span>{totalQuantity}</span></p>
                </div>
                <div className=' gst_total'>
                    <p className=' d-flex justify-content-between mb-1'>SubTotal <span>{TotalPayable.toFixed(2)}</span></p>
                    <p className=' d-flex justify-content-between mb-1'>Taxable SubTotal <span>{totalAmount}</span></p>
                    ...
                    <h3 className=' text-center my-3'>Final Amount: <span>₹ {TotalPayable.toFixed(2)}</span></h3>
                </div>
            </div>
            <div className=' text-center'>
                <button onClick={() => toPDF()} className='pdf-btn mb-5'>Download PDF</button>
            </div>
        </div>
    )
}
